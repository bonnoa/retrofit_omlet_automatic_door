...
<header>
  ...
  <div style="flex:1;text-align:center;font-family:'Space Mono',monospace;font-size:10px;color:var(--muted);letter-spacing:.5px">
    <span id="heureEsp">--:--</span>
    <span id="rssiEsp" style="margin-left:10px"></span>
  </div>
...
function connectWS() {
  ...
  ws.onopen = () => {
    ...
    ajouterLog('Connexion établie avec l\'ESP32', 'ok');
  };
...
function handleMsg(d) {
  ...
  if (d.rssi !== undefined) {
    const el = document.getElementById('rssiEsp');
    const r = d.rssi;
    let label, color;
    if (r === 0)        { label = '⚲ --';      color = 'var(--muted)'; }
    else if (r >= -60)  { label = '▮▮▮ ' + r;  color = '#3fb950'; }
    else if (r >= -70)  { label = '▮▮▯ ' + r;  color = '#d29922'; }
    else if (r >= -80)  { label = '▮▯▯ ' + r;  color = '#e0843a'; }
    else                { label = '▯▯▯ ' + r;  color = '#f85149'; }
    el.textContent = label + ' dBm';
...
void wsBroadcastState() {
  ...
  struct tm timeinfo;
  bool heureOk = getLocalTime(&timeinfo);

  HeureSoleil s = {0, 0, 0, 0};
  if (heureOk) {
    s = calculSoleil(
  ...
  char heureStr[6];
  if (heureOk) snprintf(heureStr, sizeof(heureStr), "%02d:%02d", timeinfo.tm_hour, timeinfo.tm_min);
  else         snprintf(heureStr, sizeof(heureStr), "--:--");
